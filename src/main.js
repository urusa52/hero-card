...
function freshState(seed, tutorialOn = true) {
  return {
    ...
    pendingCopy: false,
    reveal: null,   // 드라마 연출 중인 미공개 카드 { cardId, drama, text }
...
function wireIntents() {
  ...
  // 뽑힌 카드를 실제로 적용하는 공통 경로 (즉시 공개든 드라마 연출 후든 동일 로직)
  function applyDrawnCard(s, cardId) {
  ...
  // [호출] 산에서 1장 (비복원): 판돈이 큰 순간이면 공개를 지연하는 드라마 연출 (D3)
  eventBus.on('intent:draw:pull', () => {
    let scheduled = false;
    store.update((s) => {
      if (s.phase !== 'draw' || s.pullsLeft <= 0 || s.pendingCopy || s.reveal || !s.mountain.length) return;

      // 드라마 판정은 뽑기 "전"의 확률로 — 이 호출에 걸린 판돈을 시스템이 감지한다
      const odds = completionOdds(s.hand, s.mountain, data.cardById, data.yaku, data.ctx);
      const stakes = data.yaku
        .filter((d) => d.tier >= 3 && odds[d.id])
      ...
      const isDrama = stakes && s.pullsLeft <= 1;          // 마지막 즈음 + 고배율 텐파이
      const isJackpot = card.rarity >= 5;                   // 5성은 언제나 기척
      if (isDrama || isJackpot) {
        s.reveal = {
          cardId,
          drama: !!isDrama,
          text: isDrama
            ? `「${stakes.name}」까지 한 장 — 이 호출로 완성 ${odds[stakes.id].pct}%`
            : '금빛 기척이 스칩니다…',
        };
        scheduled = true; // 자동 공개 예약 (탭하면 즉시)
      } else {
        applyDrawnCard(s, cardId);
      }
    });
    if (scheduled) setTimeout(() => eventBus.emit('intent:reveal:open'), 1600);
  });

  // 리빌 공개 (탭 스킵 겸 자동 타이머 — D5: 모든 연출은 스킵 가능)
  eventBus.on('intent:reveal:open', () => store.update((s) => {
    if (!s.reveal) return;
    const { cardId } = s.reveal;
    s.reveal = null;
    applyDrawnCard(s, cardId);
  }));
